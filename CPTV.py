...
# video
cap = cv2.VideoCapture('knifeSample.mp4')
...
class WatchingStranger():
    ...
    def CheckStranger(self, frame):
        ...
        if self.fgmask.mean() != 127:
            ...
            if not self.detection and not self.tracking and (
                    self.detectFrames < (self.fps * self.detectDuration * self.tolerance)):
    ...
    def main(self, tID):
        try:
            ...
            while cap.isOpened():
                ...
                cv2.putText(frame, 'Target Place', self.originROI[0], cv2.FONT_HERSHEY_PLAIN, 2, self.roiColor,
                            2)  # test
                ...
                for i in range(len(boxes)):
                    if i in indexes:
                        ...
                        if label == 'fight':
                            ...
                            cv2.rectangle(frame, (x, y), (x + w, y + h), self.dangerColor, 3,
                                          1)  # stranger tracker 사각형 그리기, test
                            if self.socket != None:
                                self.socket.send(self.message)
                            break
...
class DetectingWeapon():
    def __init__(self, socket=None):
        self.socket = socket

        # Load Yolo(Weapon)
        self.net_weapon = cv2.dnn.readNet("yolo/yolov4-tiny-custom_best.weights", "yolo/yolov4-tiny-custom.cfg")
        # self.net_weapon.setPreferableBackent(cv2.dnn.DNN_BACKEND_OPENCV)
        # self.net_weapon.setPreferableTarget(cv2.dnn.DNN_TARGET_GPU)
        self.classes_weapon = []
        with open("yolo/obj.names", "r") as f:
            self.classes_weapon = [line.strip() for line in f.readlines()]
        self.layer_names_weapon = self.net_weapon.getLayerNames()
        self.output_layers_weapon = [self.layer_names_weapon[i[0] - 1] for i in
                                     self.net_weapon.getUnconnectedOutLayers()]

        # set rectangle color
        self.dangerColor = (0, 0, 255)

        # protocol message
        self.message = protocolMsg(id=1, level=3, length=0)

    def DetectWeapon(self):
        stime = time.time()
        ...
        while cap.isOpened():
            ...
            frame = cv2.resize(frame, None, fx=0.416, fy=0.416)
            ...
            if (time.time() - stime) % 1.5 < 1:
                ...
                self.net_weapon.setInput(blob)
                outs = self.net_weapon.forward(self.output_layers_weapon)
                ...
                for out in outs:
                    for detection in out:
                        ...
                        if confidence > 0.5:
                            # Object detected
                            # 출력 정보는 사각형 중심의 x, y좌표, 그리고 가로 세로 길이이다.
                ...
                for i in range(len(boxes)):
                    if i in indexes:
                        label = str(self.classes_weapon[class_ids[i]])

                        print(label)  # test
                        x, y, w, h = boxes[i]  # test
                        cv2.rectangle(frame, (x, y), (x + w, y + h), self.dangerColor, 3, 1)

                        if self.socket is not None:
                            self.socket.send(self.message)
                        break

            cv2.imshow('weapon', frame)  # test
            if cv2.waitKey(1) & 0xff == 27:  # test
                break  # test
